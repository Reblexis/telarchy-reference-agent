...
class Base(unittest.TestCase):
    ...
    def go(self, live=False):
        ...
        with redirect_stdout(io.StringIO()) as out:
            placed = llm_agent.run(client, live=live)
...
class TestWhatItDoesWithTheAnswer(Base):
    ...
    def test_an_answer_cut_off_after_the_numbers_still_counts(self):
        # A reasoning model that thinks for most of its budget gets truncated
        # mid-sentence in "reason". The numbers were there; use them.
        self.one_market(price=50)
        ANSWERS.append('```json\n{"value": 20, "confidence": 0.8, "reason": "The market price of 50 already inc')
        placed, _ = self.go(live=True)
        self.assertEqual(placed, 1)
        real = [t for t in TRADES if not t["dry"]]
        self.assertEqual(real[0]["body"]["targetValue"], 20)
...
class TestFirstRun(Base):
    def test_no_model_key_says_where_to_get_one_and_exits_2(self):
        with mock.patch.dict(os.environ, {"LLM_API_KEY": "", "TELARCHY_WORKSPACE": "w"}):
            err = io.StringIO()
            with mock.patch("sys.stderr", err), mock.patch("sys.argv", ["llm_agent.py"]):
                self.assertEqual(llm_agent.main(), 2)
            self.assertIn("LLM_API_KEY", err.getvalue())
            self.assertIn("logfare.ai", err.getvalue())

    def test_the_defaults_are_the_free_endpoint(self):
        with mock.patch.dict(os.environ, {"LLM_BASE_URL": "", "LLM_MODEL": ""}):
            self.assertEqual(llm_agent.base_url(), "https://logfare.ai/v1")
            self.assertEqual(llm_agent.model(), "logfare/auto")
